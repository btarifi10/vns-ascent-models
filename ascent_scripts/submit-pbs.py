...
def cluster_submit(runfibers, sim_name, sim_path, start_path_base):
    """Submit fiber simulations on a PBS-based high performance computing cluster.
    ...
    """
    pbs_params = load(os.path.join('pbs_params.json'))
    # assign params for array submission
    njobs = pbs_params['jobs_per_array'] if args.num_jobs is None else args.num_jobs
    mem = pbs_params['memory_per_fiber'] if args.job_mem is None else args.job_mem
    ...
    for tasklist in array_fibertasks:
        ...
        # Create the PBS script for submission
...
"""
        ...
        pbs_script_path = os.path.join(sim_path, 'submit.pbs')
